...
class Endpoint(BaseModel):
    ...
    auth_reason: str = Field(description="MANDATORY. Justify the security decision for THIS endpoint. State the specific code evidence: which decorator, annotation, middleware, or guard applies to this method — or which explicit public/skip/exempt rule covers it. When the global auth context shows that certain HTTP methods or path patterns are exempted (e.g. all GET requests are public), check if THIS endpoint's method and path match that exemption — if so, security MUST be [] and cite the exemption here. Always reference the actual code construct, never assume.")
